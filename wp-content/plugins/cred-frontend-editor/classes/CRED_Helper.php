...
final class CRED_Helper
{
    ...
    public static function onAdminEnqueueScripts()
    {
        ...
        $pageData=self::getAdminPage(array(
            ...
            'base'=>'admin.php',
            'pages'=>array('CRED_Forms', 'CRED_Fields', 'CRED_Settings', 'CRED_Help')
        ));

        if ( $pageData->isCustomPostEdit || $pageData->isCustomPostNew )
        {
        ...
        if ( $pageData->isPostEdit || $pageData->isPostNew || $pageData->isCustomAdminPage )
        {
        ...
        if ( $pageData->isCustomPostEdit || $pageData->isCustomPostNew || $pageData->isCustomAdminPage )
        {
            ?><style type="text/css">
                /* CRED plugin ICONS */
                ...
                <?php  if ( $pageData->isCustomPostEdit || $pageData->isCustomPostNew) { ?>
    ...
    public static function jsForCredCustomPost()
    {
        ...
        elseif ( $pageData->isPostEdit || $pageData->isPostNew )
        {
            ?>
            <script type='text/javascript'>
                /*<![CDATA[ */
        ...
        if ($no_media_button)
        {
            if (is_null($template_no_media))
            {
                ...
                if (false!==$media_button_priority)
                {
                    ...
                    array_unshift($wp_filter['media_buttons'][$media_button_priority], $media_filter_hook /*array('function' => 'media_buttons', 'accepted_args' => 1)*/);
                }
            }
